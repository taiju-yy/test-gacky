...
| 応答率（配信後24時間以内の反応） | ✅ 可能 | `getResponseRate` |
...
**応答率** は、ブロードキャスト配信後24時間以内にユーザーがメッセージを送信した割合です。

### 仕組み

1. **配信時**: ユーザーレコードに `lastBroadcastAt`（配信時刻）を記録
2. **ユーザー発言時**: 前回配信から24時間以内なら `respondedToBroadcast = true` を記録
3. **分析時**: 期間内の配信に対する反応率を集計
...
```json
{
  ...
  "totalBroadcastRecipients": 100,
  "responseCount": 25,
  "responseRate": 25.0,
  "broadcastResponseRates": [
    {
      "broadcastId": "broadcast_2025-12-29T03:34:06.916Z_18e350e3",
      "broadcastAt": "2025-12-29T03:34:06.916Z",
      "totalRecipients": 60,
      "respondedCount": 15,
      "responseRate": 25.0
    }
...
- **新規配信から有効**: この機能は実装後の配信から記録されます
- **過去の配信**: 過去の配信には `lastBroadcastAt` が記録されていないため、応答率は計算できません
- **24時間の時間枠**: デフォルトでは配信後24時間以内の反応を「応答」としてカウントします
